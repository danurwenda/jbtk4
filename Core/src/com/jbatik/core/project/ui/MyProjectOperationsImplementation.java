 ...
 */
package com.jbatik.core.project.ui;
...
/**
 * Copied from Project UI API source
 * @author Jan Lahoda
 */
public final class MyProjectOperationsImplementation {

    private static final Logger LOG = Logger.getLogger(MyProjectOperationsImplementation.class.getName());
    ...
    public static void renameProject(final Project project, final String nueName) {
        final ProgressHandle handle = ProgressHandleFactory.createHandle(NbBundle.getMessage(MyProjectOperationsImplementation.class, "LBL_Rename_Project_Handle"));
        final MyProjectRenamePanel panel = new MyProjectRenamePanel(handle, project, nueName);

        //#76559
        handle.start(MAX_WORK);

        showConfirmationDialog(panel, project, NbBundle.getMessage(MyProjectOperationsImplementation.class, "LBL_Rename_Project_Caption"), "Rename_Button", null, false, () -> {
            final String nueName1 = panel.getNewName();
            panel.addProgressBar();
            if (panel.getRenameProjectFolder()) {
                doMoveProject(handle, project, nueName1, nueName1, project.getProjectDirectory().getParent(), "ERR_Cannot_Rename");
            } else {
                project.getProjectDirectory().getFileSystem().runAtomicAction(() -> {
                    try {
                        doRenameProject(handle, project, nueName1);
                    }catch (IOException x) {
                        throw x;
                    }catch (Exception x) {
        ...
        try {
            ...
            if (originalOK) {
                ...
                if (target == null) {
                    ...
                    Exceptions.attachLocalizedMessage(e, NbBundle.getMessage(MyProjectOperationsImplementation.class, errorKey, e.getLocalizedMessage()));
                    throw e;
                }
                Project nue = ProjectManager.getDefault().findProject(target);
                if (nue != null) {
    ...
    private static void showConfirmationDialog(final JComponent panel, Project project, String caption, String confirmButton, String cancelButton, boolean doSetMessageType, final Executor executor) {
        ...
        Mnemonics.setLocalizedText(confirm, NbBundle.getMessage(MyProjectOperationsImplementation.class, "LBL_" + confirmButton));
        final JButton cancel = new JButton(cancelButton == null
                ? NbBundle.getMessage(MyProjectOperationsImplementation.class, "LBL_Cancel_Button")
                : NbBundle.getMessage(MyProjectOperationsImplementation.class, "LBL_" + cancelButton));

        confirm.getAccessibleContext().setAccessibleDescription(NbBundle.getMessage(MyProjectOperationsImplementation.class, "ACSD_" + confirmButton));
        cancel.getAccessibleContext().setAccessibleDescription(cancelButton == null
                ? NbBundle.getMessage(MyProjectOperationsImplementation.class, "ACSD_Cancel_Button")
                : NbBundle.getMessage(MyProjectOperationsImplementation.class, "ACSD_" + cancelButton));
                    ...
                    RequestProcessor.getDefault().post(() -> {
                        ...
                        try {
                            ...
                            EventQueue.invokeLater(() -> {
                                ...
                                if (e1.get() != null) {
                                    LOG.log(Level.WARNING, null, e1.get());
                                }
                            });
                        }
                    });
    ...
    private static void doRenameProject(ProgressHandle handle, Project project, String nueName) throws Exception {
        ...
        for (MoveOperationImplementation o : operations) {
            if (!(o instanceof MoveOrRenameOperationImplementation)) {
                Logger.getLogger(MyProjectOperationsImplementation.class.getName()).log(Level.WARNING,
        ...
        try {
            ...
            Exceptions.attachLocalizedMessage(e, NbBundle.getMessage(MyProjectOperationsImplementation.class, "ERR_Cannot_Rename", e.getLocalizedMessage()));
            throw e;
        }
    }
    ...
    static @CheckForNull
    String computeError(@NullAllowed File location, String projectNameText, boolean pureRename) {
        return computeError(location, projectNameText, null, pureRename);
    }

    static @CheckForNull
    String computeError(@NullAllowed File location, String projectNameText, String projectFolderText, boolean pureRename) {
        if (projectNameText.length() == 0) {
            return NbBundle.getMessage(MyProjectOperationsImplementation.class, "ERR_Project_Name_Must_Entered");
        }
        if (projectNameText.indexOf('/') != -1 || projectNameText.indexOf('\\') != -1) {
            return NbBundle.getMessage(MyProjectOperationsImplementation.class, "ERR_Not_Valid_Filename", projectNameText);
        }
        ...
        if (!location.exists()) {
            ...
            if (parent == null) {
                return NbBundle.getMessage(MyProjectOperationsImplementation.class, "ERR_Location_Does_Not_Exist");
            }
        }

        if (!parent.canWrite()) {
            return NbBundle.getMessage(MyProjectOperationsImplementation.class, "ERR_Location_Read_Only");
        }
        ...
        if (projectFolderFile.exists() && !pureRename) {
            return NbBundle.getMessage(MyProjectOperationsImplementation.class, "ERR_Project_Folder_Exists");
        }
