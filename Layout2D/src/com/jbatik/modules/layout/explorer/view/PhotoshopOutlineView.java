...
import com.jbatik.modules.layout.explorer.view.TableQuickSearchSupport.QuickSearchSettings;
...
/**
 * <p>
 * Explorer view displaying nodes in a tree table.</p>
 *
 * <p>
 * Related documentation:
 * <ul>
 * <li><a href="http://weblogs.java.net/blog/timboudreau/archive/2008/06/egads_an_actual.html">Egads!
 * An actual Swing Tree-Table!</a>
 * <li><a href="http://blogs.oracle.com/geertjan/entry/swing_outline_component">Swing
 * Outline Component</a>
 * </ul>
 *
 *
 * @author David Strupl
 */
public class PhotoshopOutlineView extends JScrollPane {

    private static final String TREE_HORIZONTAL_SCROLLBAR = "TREE_HORIZONTAL_SCROLLBAR";    // NOI18N

    private static RequestProcessor REVALIDATING_RP = new RequestProcessor("PhotoshopOutlineView", 1);   // NOI18N

    private static final Logger logger = Logger.getLogger(PhotoshopOutlineView.class.getName());
    ...
    public PhotoshopOutlineView(String nodesColumnLabel) {
    ...
    /**
     * This method allows plugging own OutlineModel to the OutlineView. You can
     * override it and create different model in the subclass.
     */
    protected OutlineModel createOutlineModel(NodeTreeModel treeModel, RowModel rowModel, String label) {
        if (label == null) {
            label = NbBundle.getMessage(PhotoshopOutlineView.class, "NodeOutlineModel_NodesColumnLabel"); // NOI18N
        }
    ...
    private void initializeTreeScrollSupport() {
        ...
        getViewport().addChangeListener((ChangeEvent e) -> {
    ...
    /**
     * Determines when the horizontal scrollbar appears in the tree column. The
     * options are:<ul>
     ...
     * @exception IllegalArgumentException if <code>policy</code> is not one of
     * the legal values shown above
     * @see #getTreeHorizontalScrollBarPolicy
     * @since 6.30
     *
     * @beaninfo preferred: true bound: true description: The tree column
     * scrollbar policy enum: HORIZONTAL_SCROLLBAR_AS_NEEDED
     ...
     */
    public PhotoshopOutline getOutline() {
        return outline;
    }
     ...
     * <i>prototypes</i> - for a given Node's property to be shown in a given
     * column, that Node must have a Property which <code>equals()</code> one of
     * the prototype properties. By default, this means that the return values
     * of the prototype property's getName() and getValueType() must exactly
     * match.
     * <p>
     * It is also possible to use the actual Property objects from one Node
     * being shown, if they are available.
     *
     * @deprecated This method is here to enable easy replacement of
     * TreeTableView with OutlineView. Use setPropertyColumns(),
     * addPropertyColumn() and removePropertyColumn() instead.
     ...
     * @param displayName A localized display name for the property which can be
     ...
     * @param displayName A localized display name for the property which can be
     * shown in the table header
     * @param description The description which will be used as a tooltip in the
     * table header
     * @since 6.25
     */
     ...
     *
     * @param name The <i>programmatic</i> name of the Property, i.e. the return
     * value of <code>Property.getName()</code>
     *
     ...
     *
     * @param namesAndDisplayNames An array, divisible by 2, of programmatic
     * name, display name, programmatic name, display name...
     * @since 6.25
     */
    ...
    /**
     * Set an attribute value for the property column representing properties
     * that have the passed programmatic (not display) name.
     *
     * @param columnName The programmatic name (Property.getName()) of the
     * column
    ...
    /**
     * Enable/disable displaying popup menus on tree view items. Default is
     * enabled.
     *
    ...
    /**
     * Enable/disable double click to invoke default action. If the default
     * action is not enabled, double click expand/collapse node.
     *
     ...
     *
     * @return <code>true</code> if the default action is invoked, or
     * <code>false</code> when it's not.
    ...
    /**
     * Test whether the quick search feature is enabled or not. Default is
     * enabled (true).
     *
     * @since 6.43
     ...
     *
     * @since 6.43
     * @param allowedQuickSearch <code>true</code> if quick search shall be
     * enabled
     */
     ...
     * @param asynchronous When <code>true</code>, the
     * {@link QuickSearchTableFilter} will be called asynchronously on a
     * background thread. When <code>false</code>, the filter will be called in
    ...
    /**
     * Method allowing to read stored values. The stored values should be only
     * those that the user has customized, it does not make sense to store the
     * values that were set using the initialization code because the
     * initialization code can be run in the same way after restart.
     */
    ...
    /**
     * Method allowing to store customization values. The stored values should
     * be only those that the user has customized, it does not make sense to
     * store the values that were set using the initialization code because the
     * initialization code can be run in the same way after restart.
     */
    ...
    private void expandSelection() {
        Node[] arr = manager.getSelectedNodes();
        for (Node arr1 : arr) {
            if ((arr1.getParentNode() == null) && (!outline.isRootVisible())) {
                // don't try to show root if it is invisible
                continue;
            }
            TreeNode tn = Visualizer.findVisualizer(arr1);
            if (tn != null) {
                ...
                Deque<TreePath> pathsStack = new ArrayDeque<>(al.size());
    ...
    private JPopupMenu createPopup(Point p) {
        ...
        ArrayList<Node> al = new ArrayList<>(selRows.length);
    ...
    /**
     * Returns the point at which the popup menu is to be shown. May return
     * null.
     *
    ...
    private Point getPositionForPopup() {
        ...
        p = SwingUtilities.convertPoint(outline, p, PhotoshopOutlineView.this);
    ...
    private class PopupAdapter extends MouseUtils.PopupMouseAdapter {
        ...
        protected void showPopup(MouseEvent e) {
            ...
            Point p = SwingUtilities.convertPoint(e.getComponent(), e.getPoint(), PhotoshopOutlineView.this);
            if (isPopupAllowed()) {
                JPopupMenu pop = createPopup(p);
                PhotoshopOutlineView.this.showPopup(p.x, p.y, pop);
                e.consume();
            }
        }
    ...
    /**
     * Check if selection of the nodes could break the selection mode set in the
     * ListSelectionModel.
     *
    ...
    private boolean isSelectionModeBroken(Node[] nodes) {
        ...
        return outline.getSelectionModel().getSelectionMode()
     ...
     *
     * @return true if dragging from the view is enabled, false otherwise.
     */
     ...
     *
     * @return true if dropping to the view is enabled, false otherwise<br>
     */
     ...
     *
     * @param state true means drops into view are allowed, false forbids any
     * drops into this view.
     */
     ...
     * actions (copy, move, link) are allowed by default.
     *
     ...
     * requested
     * @return int representing set of actions which are allowed when dropping
     * into the associated component. By default it returns
     * {@link #getAllowedDropActions()}.
     */
    ...
    /**
     * Listener attached to the explorer manager and also to the changes in the
     * table selection.
     */
    private class TableSelectionListener implements VetoableChangeListener, ListSelectionListener, PropertyChangeListener {
        ...
        public void valueChanged(javax.swing.event.ListSelectionEvent listSelectionEvent) {
            ...
            ArrayList<Node> selectedNodes = new ArrayList<>(selectedRows.length);
    ...
    private class DefaultTreeAction implements ActionListener {
        ...
        private PhotoshopOutline outline;

        DefaultTreeAction(PhotoshopOutline outline) {
    ...
    /**
     * Extension of the ETable that allows adding a special comparator for
     * sorting the rows.
     */
    static class OutlineViewOutline extends PhotoshopOutline {
        ...
        public OutlineViewOutline(final OutlineModel mdl, PropertiesRowModel rowModel) {
            ...
            setSelectVisibleColumnsLabel(NbBundle.getMessage(PhotoshopOutlineView.class, "CTL_ColumnsSelector")); //NOI18N
        ...
        private static class TextAddedTransferable implements Transferable {
            ...
            public synchronized DataFlavor[] getTransferDataFlavors() {
                if (dataFlavors == null) {
                    ...
                    List<DataFlavor> dfs = new ArrayList<>(tf.length + sf.length);
                    dfs.addAll(Arrays.asList(tf));
        ...
        public Object transformValue(Object value) {
            if (value instanceof OutlineViewOutlineColumn) {
                ...
                return NbBundle.getMessage(PhotoshopOutlineView.class, "OutlineViewOutline_NameAndDesc", dn, desc); // NOI18N
            } else if (COLUMNS_SELECTOR_HINT.equals(value)) {
                return NbBundle.getMessage(PhotoshopOutlineView.class, COLUMNS_SELECTOR_HINT);
        ...
        public boolean editCellAt(int row, int column, EventObject e) {
            ...
            if (o instanceof Node.Property) { // && (e == null || e instanceof KeyEvent)) {
                //=======================MODIFIED??============================
                //in this photoshop style, editing at a property cell won't trigger selection change
            ...
            if (isTreeColumn && row != -1 && e instanceof MouseEvent && SwingUtilities.isLeftMouseButton((MouseEvent) e)) {
                //di sini cuma handle checkAt di tree column, tidak di kolom lain
                ...
                if (clickCount > 1) {
                    // Default action.
                    // dimas : double-click on tree column = default action
                    // nyatanya default action ini adalah expand/collapse jika bukan leaf
                    // dan rename jika leaf
                        ...
                        if (defaultActionAllowed) {
                            ...
                            if (a != null) {
                                if (a.isEnabled()) {
                                    ...
                                    Logger.getLogger(PhotoshopOutlineView.class.getName()).info("Action " + a + " on node " + node + " is disabled");
                                }
                            }
                        }
                    }
            ...
            if (e instanceof MouseEvent) {
                ...
                // dimas : 24 here is the width of that small rectangle on the right end of cell
        ...
        /**
         * Extension of ETableColumn using TableViewRowComparator as comparator.
         */
    ...
    private final class ScrollListener extends ComponentAdapter implements ChangeListener,
        ...
        private void revalidateScrollBar() {
            ...
            if ((outline.getColumnModel().getColumnCount() > 0)) {
    ...
    private static class OutlineScrollLayout extends ScrollPaneLayout.UIResource {
        ...
        public Dimension preferredLayoutSize(Container parent) {
            ...
            PhotoshopOutlineView ov = (PhotoshopOutlineView) parent;
