 ...
 * <P>
 * There is at most one VisualizerNode for one node. All of them are held in a
 * cache.
 * <P>
 * VisualizerNode provides a thread-safe layer between Nodes, which may fire
 * property changes on any thread, and the AWT dispatch thread thread.
 * 
...
final class VisualizerNode extends EventListenerList implements NodeListener, TreeNode, Runnable {
    ...
    /**
     * node prefetch count before initialization of VisualizerChildren - to
     * avoid
     */
    ...
    /**
     * visualizer children attached thru weak references Reference
     * (VisualizerChildren)
     */
    ...
    /**
     * Update the state of this class by retrieving new name, etc. And fire
     * change to all listeners. Only by AWT-Event-Queue
     */
    public void run() {
        if (!Children.MUTEX.isReadAccess()) {
    ...
    /**
     * Class that processes runnables in event queue. It guarantees that the
     * order of processed objects will be exactly the same as they arrived.
     */
    private static final class QP {

        /**
         * queue of all requests (Runnable) that should be processed AWT-Event
         * queue.
         */
    ...
    /**
     * Builds the parents of vis. node up to and including the root node from
     * VisualizerNode hierarchy
     */
