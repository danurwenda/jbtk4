...
class TreeViewCellEditor extends DefaultTreeCellEditor implements CellEditorListener, FocusListener,
    ...
    /**
     * Stopped is true, if the editation is over (editingStopped is called for
     * the first time). The two variables have virtually the same function, but
     * are kept separate for code clarity.
     */
     ...
     * i.e. cell editor, it cancels editing, otherwise it calls superclass
     * method.
     */
    @Override
    ...
    /**
     * * Sets the state od drag and drop operation. It's here only because of
     * JTree's bug which allows to start the editing even if DnD operation
     * occurs (bug # )
     */
