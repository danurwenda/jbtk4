...
public abstract class TreeView extends JScrollPane {
    ...
    /**
     * How long it takes before collapsed nodes are released from the tree's
     * cache
     */
     ...
     * Constructor.
     *
     * @param defaultAction should double click on a node open its default
     * action?
...
//        }
    ...
    /**
     * Initializes the tree & model. [dafe] Horrible technique - overridable
     * method called from constructor may result in subclass code invoked when
     * this object is not fully constructed. However I don't have enough
    ...
    /**
     * Enable/disable double click to invoke default action. If defaultAction is
     * not enabled double click expand/collapse node.
     *
     * @param value <code>true</code> to enable
     */
    ...
    /**
     * Set whether or not the root node from the <code>TreeModel</code> is
     * visible.
     *
    ...
    /**
     * Set whether the quick search feature uses substring or prefix matching
     * for the typed characters. Defaults to prefix (false).
     *
     * @since 6.11
     * @param useSubstring <code>true</code> if substring search is used in
     * quick search
     * @deprecated Since 6.42 does nothing, as the quick search feature uses
     * both substring and prefix matching. All prefix matches for the typed
     * characters will appear before substring matches.
     */
    @Deprecated
    ...
    /**
     * Expands the node in the tree. This method can be called outside of AWT
     * dispatch thread. It gets the children fro the node immediately, and then
     * switches to AWT via {@link EventQueue#invokeLater(java.lang.Runnable)}
    ...
    /**
     * Called to allow subclasses to define the behaviour when a node(s) are
     * selected in the tree.
     *
     * @param nodes the selected nodes
     * @param em explorer manager to work on (change nodes to it)
     * @throws PropertyVetoException if the change cannot be done by the
     * explorer (the exception is silently consumed)
     */
    ...
    /**
     * Show a given path in the screen. It depends on the kind of
     * <code>TreeView</code> if the path should be expanded or just made
     * visible.
     *
     * @param path the path
     */
     ...
     * Applicable when no nodes are selected and the user wants to invoke a
     * context menu (clicks right mouse button).
     *
     * @return <code>true</code> if so; <code>false</code> in the default
     * implementation
     */
    ...
    /**
     * Check if selection of the nodes could break the selection mode set in
     * TreeSelectionModel.
     *
     ...
     * is changed to <code>TreeSelectionModel.SINGLE_TREE_SELECTION</code>, only
     * one TreePath will remain selected. It is up to the particular
     * implementation to decide what TreePath remains selected. Note:
     * TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION is set as default.
     *
     * @since 2.15
    ...
    /**
     * Synchronize the selected nodes from the manager of this Explorer. The
    ...
    /**
     * Returns the the point at which the popup menu is to be showed. May return
     * null.
     *
    ...
    /**
     * Returns the tree path nearby to given tree node. Either a sibling if
     * there is or the parent.
     *
    ...
    class TreePropertyListener implements VetoableChangeListener, PropertyChangeListener, TreeExpansionListener,
        ...
        public @Override
        final void propertyChange(PropertyChangeEvent evt) {
            if (manager == null) {
    ...
    private final class ExplorerTree extends JTree implements Autoscroll, QuickSearch.Callback {
        ...
        /**
         * removeNotify() call count sometimes does not match addNotify(), use
         * special flag
         */
...
//            }
         ...
         *
         * @return An instance of SubstringSearchResult containing the matching
         * TreePath and the index of the first occurrence of the substring in
         * TreePath.
         */
