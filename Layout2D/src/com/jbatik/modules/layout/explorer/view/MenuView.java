...
/**
 * An explorer view that shows the context hierarchy in a popup menu. Initially,
 * it shows a left button which opens a popup menu from the root context and a
 * right button which opens a popup menu from the currently explored context.
 *
 * <p>
 * This class is a <q>view</q>
 * to use it properly you need to add it into a component which implements
 * {@link Provider}. Good examples of that can be found in
 * {@link ExplorerUtils}. Then just use {@link Provider#getExplorerManager} call
 * to get the {@link ExplorerManager} and control its state.
 * </p>
 * <p>
 * There can be multiple <q>views</q> under one container implementing
 * {@link Provider}. Select from range of predefined ones or write your own:
 * </p>
 * <ul>
 * <li>{@link com.jbatik.modules.layout.explorer.view.BeanTreeView} - shows a
 * tree of nodes</li>
 * <li>{@link com.jbatik.modules.layout.explorer.view.ContextTreeView} - shows a
 * tree of nodes without leaf nodes</li>
 * <li>{@link com.jbatik.modules.layout.explorer.view.ListView} - shows a list
 * of nodes</li>
 * <li>{@link com.jbatik.modules.layout.explorer.view.IconView} - shows a rows
 * of nodes with bigger icons</li>
 * <li>{@link com.jbatik.modules.layout.explorer.view.ChoiceView} - creates a
 * combo box based on the explored nodes</li>
 * <li>{@link com.jbatik.modules.layout.explorer.view.TreeTableView} - shows
 * tree of nodes together with a set of their {@link Property}</li>
 * <li>{@link com.jbatik.modules.layout.explorer.view.MenuView} - can create a
 * {@link JMenu} structure based on structure of {@link Node}s</li>
 * </ul>
 * <p>
 * All of these views use {@link ExplorerManager#find} to walk up the AWT
 * hierarchy and locate the {@link ExplorerManager} to use as a controler. They
 * attach as listeners to it and also call its setter methods to update the
 * shared state based on the user action. Not all views make sence together, but
 * for example {@link com.jbatik.modules.layout.explorer.view.ContextTreeView}
 * and {@link com.jbatik.modules.layout.explorer.view.ListView} were designed to
 * complement themselves and behaves like windows explorer. The
 * {@link org.openide.explorer.propertysheet.PropertySheetView} for example
 * should be able to work with any other view.
 * </p>
 *
 *
...
public class MenuView extends JPanel {
    ...
    /**
     * Acceptor that can be passed to constructor of {@link MenuView.Menu}. It
     * permits determination of which nodes should be accepted upon a click.
     *
     * @deprecated This interface is almost the same as {@link NodeAcceptor} so
     * it is redundant and obsoleted. Use {@link NodeAcceptor} interface
     * instead.
     */
    ...
    interface Acceptor {
        ...
        /**
         * Test whether to accept the node or not. Can also perform some actions
         * (such as opening the node, etc.).
         *
         * @param n the node
         * @return true if the <code>menu</code> should close
         * @deprecated whole interface is obsoleted, use
         * {@link NodeAcceptor#acceptNodes} instead.
         */
        public @Deprecated
    ...
    /**
     * Menu item representing a node (with children) in a menu hierarchy. One
     * can attach an acceptor to the menu that will be informed each time a user
     * selects an item whether to close the menu or not. The submenu content is
     * taken as a blocking snapshot of the Node's Children at the time of the
     * submenu popup and does not reflect subsequent changes to Node's Children
     * until next popup. This means that the Node's Children should properly
     * implement blocking getNodes(true) in order to show proper content in
     * MenuView.
     */
    public static class Menu extends org.openide.awt.JMenuPlus {
        ...
        /**
         * Constructor that assigns the node a default action. For example, open
         * the Explorer or a property sheet.
         *
        ...
        /**
         * @deprecated use
         * {@link MenuView.Menu#MenuView.Menu(Node, NodeAcceptor)}
         */
        public @Deprecated
        Menu(Node node, Acceptor action) {
            this(node, new AcceptorProxy(action), true);
        }

        /**
         * @deprecated use
         * {@link MenuView.Menu#MenuView.Menu(Node, NodeAcceptor, boolean)}
         */
        public @Deprecated
        Menu(Node node, Acceptor action, boolean setName) {
            this(node, new AcceptorProxy(action), setName);
        }

        /**
         * Constructor that permits specification of the action on the node, and
         * permits overriding the name and icon of the menu.
         *
         * @param node node to represent
         * @param action action called when node selected
         * @param setName <code>true</code> to automatically set the name and
         * icon of the item
         */
        ...
        /**
         * Checks for {@link MouseEvent#isPopupTrigger right click} to ask the
         * acceptor whether to accept the selection.
         *
         ...
         * {@link MenuView.MenuItem}s for leafs and <code>Menu</code> for other
         * nodes.
         *
    ...
    /**
     * Proxy that allows to use now deprecated MenuView.Acceptor interface on
     * places where NodeAcceptor is requested. This class can be deleted
     * together with MenuView.Acceptor deletion.
     */
    ...
    public static class MenuItem extends JMenuItem implements HelpCtx.Provider {
         ...
         */
        public @Deprecated
        MenuItem(Node node, Acceptor action) {
            this(node, new AcceptorProxy(action), true);
        }
         ...
         */
        public @Deprecated
        MenuItem(Node node, Acceptor action, boolean setName) {
            this(node, new AcceptorProxy(action), setName);
        }

        /**
         * Construct item for given node, specifying the action and whether to
         * create the icon and name automatically.
         *
         * @param node the node to represent
         * @param l the acceptor to decide whether to accept this node or not
         * @param setName <code>false</code> if the name and icon should not be
         * set
         */
