...
/**
 * An Outline, or tree-table component. Takes an instance of OutlineModel, an
 * interface which merges TreeModel and TableModel.
 * <p>
 * Simplest usage:
 * <ol>
 * <li>Create a standard tree model for the tree node portion of the
 * outline.</li>
 * <li>Implement RowModel. RowModel is a subset of TableModel - it is passed the
 * value in column 0 of the Outline and a column index, and returns the value in
 * the column in question.</li>
 * <li>Pass the TreeModel and the RowModel to
 * <code>DefaultOutlineModel.createModel()</code>
 * </ol>
 ...
 * <p>
 * It is also useful to provide an implementation of
 * <code>RenderDataProvider</code> to supply icons and affect text display of
 * cells - this covers most of the needs for which it is necessary to write a
 * custom cell renderer in JTable/JTree.
 * <p>
 * <b>Example usage:</b><br>
 * Assume FileTreeModel is a model which, given a root directory, will expose
 * the files and folders underneath it. We will implement a RowModel to expose
 * the file size and date, and a RenderDataProvider which will use a gray color
 * for uneditable files and expose the full file path as a tooltip. Assume the
 * class this is implemented in is a JPanel subclass or other Swing container.
 * <br>
 ...
 *
 * <p>
 * <b>Note:</b> This API is still under development and may change even in
 * incompatible way during its stabilization phase. The API will be finalized in
 * NetBeans version 6.5.</p>
 *
 * @author Tim Boudreau
 */
//dimas - implements autoscroll
public class PhotoshopOutline extends ETable implements Autoscroll {
     ...
     */
    public PhotoshopOutline() {
        init();
    }

    public PhotoshopOutline(OutlineModel mdl) {
        super(mdl);
        init();
    }
    ...
    private void init() {
        initialized = true;
        setDefaultRenderer(Object.class, new PhotoshopOutlineCellRenderer());
     ...
     * for items in the tree column. The default is null. If null, the data
     * displayed will be generated in the standard JTable/JTree way - calling
     * <code>toString()</code> on objects in the tree model and using the look
     ...
     * <strong>Under no circumstances directly call the methods on the layout
     * cache which change the expanded state - such changes will not be
     * propagated into the table model, and will leave the model and its layout
     * in inconsistent states. Any calls that affect expanded state must go
     * through <code>getTreePathSupport()</code>.</strong>
     */
    ...
    public void setRowHeight(int row, int rowHeight) {
        Logger.getLogger(PhotoshopOutline.class.getName()).warning("Not supported yet."); // NOI18N
    }
    ...
    protected void sortAndFilter() {
        ...
        if (tcm instanceof ETableColumnModel) {
            ...
            Comparator<ETable.RowMapping> c = etcm.getComparator();
            if (c != null) {
                TableModel model = getModel();
                int noRows = model.getRowCount();
                List<ETable.RowMapping> rows = new ArrayList<ETable.RowMapping>();
                synchronized (tempSortMapLock) {
                    if (tempSortMap != null) {
                        return; // Sorting right now
                    }
                    Map<TreePath, ETable.RowMapping> tsm = new HashMap<TreePath, ETable.RowMapping>();
                ...
                for (int i = 0; i < res.length; i++) {
                    ETable.RowMapping rm = rows.get(i);
    ...
    protected class OutlineColumn extends ETableColumn {
        ...
        public OutlineColumn(int modelIndex) {
            super(modelIndex, PhotoshopOutline.this);
        }
        ...
        /**
         * Comparator used for sorting the rows according to value in a given
         * column. Operates on the RowMapping objects.
         */
    ...
    public boolean editCellAt(int row, int column, EventObject e) {
        ...
        if (isTreeColumn && e instanceof MouseEvent) {
            ...
            if (!getOutlineModel().isLeaf(path.getLastPathComponent())) {
                //is not leaf, check for clicking handle
                int handleWidth = PhotoshopOutlineCellRenderer.getExpansionHandleWidth();
                Insets ins = getInsets();
                int nd = path.getPathCount() - (isRootVisible() ? 1 : 2);
                if (nd < 0) {
                    nd = 0;
                }
                int handleStart = ins.left + (nd * PhotoshopOutlineCellRenderer.getNestingWidth());
                ...
                //================MODIFIED================================
                //we shift the handle location if there's a checkbox
                RenderDataProvider render = getRenderDataProvider();
                TableCellRenderer tcr = getDefaultRenderer(Object.class);
                if (render instanceof CheckRenderDataProvider && tcr instanceof PhotoshopOutlineCellRenderer) {
                    CheckRenderDataProvider crender = (CheckRenderDataProvider) render;
                    PhotoshopOutlineCellRenderer ocr = (PhotoshopOutlineCellRenderer) tcr;
                ...
                if ((me.getX() > ins.left && me.getX() >= handleStart && me.getX() <= handleEnd)
                    ...
                    //expanding a tree won't make the node selected
        ...
        if (!isTreeColumn || e instanceof MouseEvent && isEditEvent(row, column, (MouseEvent) e)) {
            //===========================MODIFIED??==============================
            //in this photoshop style, selection changes ONLY triggered by clicking
            //on the tree column, outside the checkbox AND outside the handle
            //OR
            //using arrow button
            //that means clicking on any property column won't trigger the selection change
    ...
    /**
     * Perform a selection/deselection of a check box on the given row and
     * column, if a check box exists on the given position.
     *
     * Note that it's only called to check the event happened on tree column.
     ...
     * on the given row and column, is checkable and enabled and the mouse event
     * is either <code>null</code> or upon the check-box location. Returns
    ...
    protected final boolean checkAt(int row, int column, MouseEvent me) {
        ...
        if (render instanceof CheckRenderDataProvider && tcr instanceof PhotoshopOutlineCellRenderer) {
            ...
            if (crender.isCheckable(value) && crender.isCheckEnabled(value)) {
                boolean isChBoxPosition;
                if (me == null) {
                    //dimas : handle SPACE to check/uncheck
                    isChBoxPosition = true;
                } else {
                    //================================MODIFIED=================================
                    //in this photoshop mode, all checkbox is rendered in a left aligned fashion
                    int chWidth = ocr.getTheCheckBoxWidth();
                    Insets ins = getInsets();

                    int chStart = ins.left;
                    int chEnd = chStart + chWidth;

                    isChBoxPosition = (me.getX() > ins.left && me.getX() >= chStart && me.getX() <= chEnd);
                }
                if (isChBoxPosition) {
                    //================================MODIFIED=================================
                    if (me != null) {
                        //user really click on checkbox
                        //in photoshop mode, it won't trigger selection change
                        selectionDisabled = true;
                    }
    ...
    /**
     * Configure the cell editor. This method allows to override the
     * configuration of cell editor when cell editing is initiated.
     *
    ...
    private void calcRowHeight() {
        ...
        if (i != null) {
            ...
            int h = Math.max(fm.getHeight() + fm.getMaxDescent(),
                    PhotoshopOutlineCellRenderer.getExpansionHandleHeight());
...
//        System.err.println("row count is " + getRowCount());
    ...
    /**
     * Create a component listener to handle size changes if the table model is
     * large-model
     */
    ...
    private class ND extends AbstractLayoutCache.NodeDimensions {
        ...
        public Rectangle getNodeDimensions(Object value, int row, int depth,
            ...
            int wid = PhotoshopOutline.this.getColumnModel().getColumn(0).getPreferredWidth();
    ...
    /**
     * A component listener. If we're a large model table, we need to inform the
     * FixedHeightLayoutCache when the size changes, so it can update its
     * mapping of visible nodes
     */
    ...
    private static class TreeCellEditorBorder implements Border {
        ...
        public Insets getBorderInsets(Component c) {
            insets.left = (nestingDepth
                    * PhotoshopOutlineCellRenderer.getNestingWidth())
                    + PhotoshopOutlineCellRenderer.getExpansionHandleWidth() + 1;
        ...
        public void paintBorder(Component c, java.awt.Graphics g, int x, int y, int width, int height) {
            int iconY;
            int iconX = nestingDepth * PhotoshopOutlineCellRenderer.getNestingWidth();
            if (!isLeaf) {
                Icon expIcon = isExpanded
                        ? PhotoshopOutlineCellRenderer.getExpandedIcon()
                        : PhotoshopOutlineCellRenderer.getCollapsedIcon();
            ...
            if (null != icon) {
                iconX += PhotoshopOutlineCellRenderer.getExpansionHandleWidth() + 1;
