...
/**
 * This class provide access to thread safe layer that reflects the hierarchy of
 * Nodes, but is updated only in event dispatch thread (in contrast to nodes
 * that can be updated from any thread). That is why this class is useful for
 * writers of explorer views, because it guarantees that all changes will be
 * done safely.
 * <P>
...
public class Visualizer extends Object {
    ...
    /**
     * Methods that create a tree node for given node. The tree node reflects
     * the state of the associated node as close as possible, but is updated
     * asynchronously in event dispatch thread.
     * <P>
