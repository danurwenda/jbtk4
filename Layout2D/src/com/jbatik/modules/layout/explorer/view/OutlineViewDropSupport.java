...
final class OutlineViewDropSupport implements DropTargetListener, Runnable {
    ...
    /**
     * Node area which we were during DnD operation.
     */
     ...
     * View manager.
     */
    protected PhotoshopOutlineView view;
     ...
     */
    public OutlineViewDropSupport(PhotoshopOutlineView view, JTable table, boolean dropTargetPopupAllowed) {
        this.view = view;
        this.table = table;
    ...
    private void doDragOver(DropTargetDragEvent dtde) {
        ...
        // ADDED BY DIMAS, always RESET L,U,D
        lowerNodeIdx = -1;
        upperNodeIdx = -1;
        dropIndex = -1;
        //11/29/14 these 3 lines solve the problem of unnecessary reordering on dropping ON node
        ...
        if (((timer == null) || !timer.isRunning()) && (dropNode != null) && !dropNode.isLeaf()
                && !expanded) {
    ...
    /**
     * Repaints TreeView, the given rectangle is enlarged for 5 pixels because
     * some parts was not repainted correctly.
     *
    ...
    /**
     * Converts line's bounds by the bounds of the root pane. Drop glass pane is
     * over this root pane. After covert a given line is set to drop glass pane.
     *
    ...
    public void dropActionChanged(DropTargetDragEvent dtde) {
        ...
        for (int i = 0; i < nodes.length; i++) {
            if (((view.getAllowedDropActions(dtde.getTransferable()) & dropAction) == 0)
    ...
    /**
     * Performs the drop action, if we are dropping on right node and target
     * node agrees.
     */
    ...
    /**
     * Activates or deactivates Drag support on asociated JTree component
     *
     * @param active true if the support should be active, false otherwise
     */
    ...
    /**
     * Implementation of the runnable interface. Notifies user in AWT thread.
     */
    public void run() {
    ...
    /**
     * Safe accessor to the drop target which is asociated with the table
     */
